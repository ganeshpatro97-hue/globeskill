      ...
      <div className="bg-gradient-to-r from-rose-900 via-emerald-900 to-slate-900 rounded-3xl p-8 sm:p-12 text-white shadow-md relative overflow-hidden">
        <div className="max-w-3xl relative z-10 space-y-4">
          <div className="inline-flex items-center gap-2 bg-rose-500/20 backdrop-blur-md border border-rose-400/30 px-3.5 py-1 rounded-full text-xs font-bold text-rose-200">
            <Heart className="w-3.5 h-3.5 text-rose-400 fill-rose-400" /> Empower India&apos;s Youth with Digital &amp; AI Skills
          </div>

          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight leading-tight">
            Help Us Bridge the AI &amp; Digital Opportunity Gap
          </h1>

          <p className="text-sm sm:text-base text-slate-200 leading-relaxed max-w-2xl">
            100% of your tax-deductible contribution directly sponsors laptops, localized AI curriculum, and hands-on coding mentors for underserved children.
          </p>

          <div className="pt-2 flex flex-wrap items-center gap-4 text-xs text-slate-300">
            <span className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-lg">
              <ShieldCheck className="w-4 h-4 text-emerald-400" /> Section 80G Tax Exemption
            </span>
            <span className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-lg">
              <Users className="w-4 h-4 text-teal-300" /> Direct Student Cohort Matching
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <span className="text-xs font-extrabold uppercase tracking-wider text-emerald-800 bg-emerald-50 px-2.5 py-0.5 rounded-full">
              Phase 1-5 National Campaign Goal
            </span>
            <h2 className="text-xl font-bold text-slate-900 mt-1">
              ₹{stats ? stats.totalFundsRaised.toLocaleString('en-IN') : '1,75,000'} Raised of ₹5,00,000 Target
            </h2>
          </div>
          <div className="text-right">
            <span className="text-2xl font-black text-emerald-700">{progressPercent}%</span>
            <span className="text-xs text-slate-500 block">of ₹5 Lakh Goal</span>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-xs">
            ...
            {successReceiptUrl ? (
              <div className="text-center py-8 space-y-4 animate-in fade-in">
                ...
                <h3 className="text-2xl font-extrabold text-slate-900">Thank You for Your Support! ❤️</h3>
                <p className="text-xs sm:text-sm text-slate-600 max-w-md mx-auto leading-relaxed">
                  Your generous contribution of <strong>₹{effectiveAmount.toLocaleString('en-IN')}</strong> has been received with transaction ID <code className="font-mono bg-slate-100 px-1.5 py-0.5 rounded text-xs">{successTxnId}</code>.
                </p>
                <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-colors"
                  >
              ...
              <form onSubmit={handleDonate} className="space-y-6">
                ...
                <div>
                    ...
                    {TIERS.map((tier) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className="text-xs font-bold text-emerald-900 mt-1">{tier.label}</p>
                  ...
                  <div className="mt-3">
                    <input
                      type="number"
                      placeholder="Or enter custom amount in INR (e.g. 5000)"
                      value={customAmount}
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">
                    2. Choose Program / Cause to Support
                  </label>
                  <select
                    value={selectedCause}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">Donor Name / Org</label>
                    <input
                      type="text"
                      required
                      value={donorName}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">Email (for 80G Receipt)</label>
                    <input
                      type="email"
                      required
                      value={donorEmail}
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">
                    3. Payment Channel (Secure 256-Bit SSL)
                  </label>
                ...
                >
                  ...
                  {processing ? 'Processing Secure Transaction...' : `Donate ₹${effectiveAmount.toLocaleString('en-IN')} & Generate 80G Receipt`}
                </button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
              <ShieldCheck className="w-4 h-4 text-emerald-600" /> 100% Tax Deductible
            </h3>
            <p className="text-xs text-slate-600 leading-relaxed">
              GlobeSkill is registered under Section 80G of the Income Tax Act. Donors in India receive a 50% tax exemption receipt instantly upon checkout.
            </p>
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-[11px] text-slate-600 space-y-1">
              <div><strong>Registration No:</strong> AAATG1234F20261</div>
              <div><strong>Audit Standard:</strong> ISO 9001 Non-Profit Compliance</div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-emerald-50 via-teal-50 to-white rounded-2xl border border-emerald-200 p-6 shadow-xs space-y-3">
            ...
            <ul className="space-y-2 text-xs text-slate-700">
              <li className="flex items-center gap-2">
                ...
                <span><strong>85%</strong> Direct Learning Hardware &amp; AI Kits</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-teal-500"></span>
                <span><strong>10%</strong> Trainer Stipends &amp; Classroom Labs</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-blue-500"></span>
                <span><strong>5%</strong> Operations &amp; Cloud Infrastructure</span>
              </li>
            </ul>
          </div>
